...
echo "Peter Tkatchenko"   # please fill in names here
echo "My apologies; I'll find a second person to work with on the next assignment"
...
# 1. use the `awk` command to separt each row at the `::` delimiters
...
awk -F '::' '{print $3}' movies.dat | awk '{print split($0, a, "\\|")}' | sort | uniq -c > ~/scpoprogramming/hw1/outtable.txt
